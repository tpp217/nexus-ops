 ...
 *
 * - 検証済みクレームに tenant_id があればそれを使う。
 * - 解決できなければ AUTH_ENFORCE の値に関わらず fail-closed（{ ok:false }）。
 *   以前は監視モードで utinc 既定にフォールバックしていたが、AUTH_ENFORCE を外すだけで
 *   未認証の誰でも utinc の議事録を読み書きできる状態になるため廃止した。
 *
