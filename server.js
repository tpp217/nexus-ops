...
app.post('/api/analyze', async (req, res) => {
  ...
  // 新仕様: セクション見出し + 箇条書き構造で返してもらう（ai_review は改行区切りの文字列）
  const instructions =
    ...
    `ai_reviewの書式ルール（厳守）:\n` +
    `・以下5つのセクションを順に出力すること\n` +
    `  【課題】【状態】【モチベーション】【原因と結果】【アクション】\n` +
    `・各セクション見出しの直後に改行し、1〜3項目を「・」で始まる箇条書きで書く\n` +
    `・1項目は1〜2文、合計で600字前後に収めること\n` +
    `・前置き・自己紹介・入力の丸写しは禁止\n` +
    `・同じ内容を別セクションに繰り返さないこと\n\n` +
    `以下のJSON形式のみで返してください（説明文・前置き不要、Markdown不可）:\n` +
    `{"ai_review":"【課題】\\n・〜\\n・〜\\n【状態】\\n・〜\\n【モチベーション】\\n・〜\\n【原因と結果】\\n・〜\\n【アクション】\\n・〜",` +
...
app.post('/api/analyze/person', async (req, res) => {
  ...
  const instructions =
    ...
    `ai_reviewの書式ルール（厳守）:\n` +
    `・以下7つのセクションを順に出力すること\n` +
    `  【総合評価】【成長の軌跡】【一貫した強み】【継続課題】【改善傾向】【懸念】【今後の方針】\n` +
    `・各セクション見出しの直後に改行し、1〜3項目を「・」で始まる箇条書きで書く\n` +
    `・1項目は1〜2文、合計で900字前後に収めること\n` +
    `・前置き・自己紹介・入力の丸写しは禁止\n` +
    `・同じ内容を別セクションに繰り返さないこと\n\n` +
    `以下のJSON形式のみで返してください（説明文・前置き不要、Markdown不可）:\n` +
    `{"ai_review":"【総合評価】\\n・〜\\n【成長の軌跡】\\n・〜\\n【一貫した強み】\\n・〜\\n【継続課題】\\n・〜\\n【改善傾向】\\n・〜\\n【懸念】\\n・〜\\n【今後の方針】\\n・〜",` +
